...
class Gateway extends Core_Gateway {
	...
	public function __construct( Config $config ) {
		...
		$field_consumer_name->set_label( __( 'Account holder name', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		...
		$field_consumer_iban->set_label( __( 'Account number (IBAN)', 'pronamic-pay-with-mollie-for-woocommerce' ) );
	...
	public function maybe_create_shipment_for_payment( Payment $payment ): void {
			...
			if ( $mollie_payment_id !== $transaction_id ) {
				...
				$payment->add_note(
					...
					/* translators: 1: payment transaction ID, 2: Mollie payment ID */
						\__( 'Payment transaction ID updated from `%1$s` to successful order payment `%2$s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
						$transaction_id,
						$mollie_payment_id
					)
				);
		...
		$payment->add_note(
			...
			/* translators: 1: Mollie shipment ID, 2: Mollie order ID */
				\__( 'Shipment `%1$s` created for Mollie order `%2$s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
				$shipment->get_id(),
				$mollie_order_id
			)
		);
					...
					if ( $mollie_chargeback->get_created_at() > $subscription->get_activated_at() ) {
						...
						$subscription->add_note(
							...
							/* translators: 1: Mollie chargeback ID, 2: Mollie payment ID */
								\__( 'Subscription put on hold due to chargeback `%1$s` of payment `%2$s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function update_subscription_mandate( Subscription $subscription, $mandate_id, $payment_method = null ) {
		...
		if ( ! empty( $old_mandate_id ) && $old_mandate_id !== $mandate_id ) {
			...
			/* translators: 1: old mandate ID, 2: new mandate ID */
				\__( 'Mandate for subscription changed from "%1$s" to "%2$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		if ( ! empty( $old_method ) && $old_method !== $new_method ) {
			...
			$note = \sprintf(
				...
				\__( 'Payment method for subscription changed from "%1$s" to "%2$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
