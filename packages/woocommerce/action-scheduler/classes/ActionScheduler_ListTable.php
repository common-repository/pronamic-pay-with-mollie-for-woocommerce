...
class ActionScheduler_ListTable extends ActionScheduler_Abstract_ListTable {
	...
	public function __construct( ActionScheduler_Store $store, ActionScheduler_Logger $logger, ActionScheduler_QueueRunner $runner ) {
		...
		$this->table_header = __( 'Scheduled Actions', 'pronamic-pay-with-mollie-for-woocommerce' );

		$this->bulk_actions = array(
			'delete' => __( 'Delete', 'pronamic-pay-with-mollie-for-woocommerce' ),
		);

		$this->columns = array(
			'hook'        => __( 'Hook', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'status'      => __( 'Status', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'args'        => __( 'Arguments', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'group'       => __( 'Group', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'recurrence'  => __( 'Recurrence', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'schedule'    => __( 'Scheduled Date', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'log_entries' => __( 'Log', 'pronamic-pay-with-mollie-for-woocommerce' ),
		);
		...
		if ( empty( $request_status ) ) {
			...
			$this->columns  += array( 'claim_id' => __( 'Claim ID', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		...
		$this->row_actions = array(
			'hook' => array(
				'run' => array(
					'name'  => __( 'Run', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'desc'  => __( 'Process the action now as if it were run as part of a queue', 'pronamic-pay-with-mollie-for-woocommerce' ),
				),
				'cancel' => array(
					'name'  => __( 'Cancel', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'desc'  => __( 'Cancel the action now to avoid it being run in future', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'class' => 'cancel trash',
				),
			),
		);
		...
		self::$time_periods = array(
			array(
				...
				'names'   => _n_noop( '%s year', '%s years', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
			array(
				'seconds' => MONTH_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s month', '%s months', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
			array(
				'seconds' => WEEK_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s week', '%s weeks', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
			array(
				'seconds' => DAY_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s day', '%s days', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
			array(
				'seconds' => HOUR_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s hour', '%s hours', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
			array(
				'seconds' => MINUTE_IN_SECONDS,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s minute', '%s minutes', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
			array(
				'seconds' => 1,
				/* translators: %s: amount of time */
				'names'   => _n_noop( '%s second', '%s seconds', 'pronamic-pay-with-mollie-for-woocommerce' ),
			),
		);
	...
	private static function human_interval( $interval, $periods_to_include = 2 ) {
		...
		if ( $interval <= 0 ) {
			return __( 'Now!', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
		...
		for ( $time_period_index = 0, $periods_included = 0, $seconds_remaining = $interval; $time_period_index < count( self::$time_periods ) && $seconds_remaining > 0 && $periods_included < $periods_to_include; $time_period_index++ ) {
			...
			if ( $periods_in_interval > 0 ) {
				...
				$output .= sprintf( translate_nooped_plural( self::$time_periods[ $time_period_index ]['names'], $periods_in_interval, 'pronamic-pay-with-mollie-for-woocommerce' ), $periods_in_interval );
	...
	protected function get_recurrence( $action ) {
		...
		if ( $schedule->is_recurring() && method_exists( $schedule, 'get_recurrence' ) ) {
			...
			if ( is_numeric( $recurrence ) ) {
				...
				return sprintf( __( 'Every %s', 'pronamic-pay-with-mollie-for-woocommerce' ), self::human_interval( $recurrence ) );
			} else {
				return $recurrence;
			}
		}

		return __( 'Non-repeating', 'pronamic-pay-with-mollie-for-woocommerce' );
	}
	...
	public function display_admin_notices() {
			...
			foreach ( $table_list as $table_name ) {
				if ( ! in_array( $wpdb->prefix . $table_name, $found_tables ) ) {
					$this->admin_notices[] = array(
						'class'   => 'error',
						'message' => __( 'It appears one or more database tables were missing. Attempting to re-create the missing table(s).' , 'pronamic-pay-with-mollie-for-woocommerce' ),
					);
		...
		if ( $this->runner->has_maximum_concurrent_batches() ) {
			...
			if ( false === $async_request_lock_expiration || $async_request_lock_expiration < time() ) {
				...
				$async_request_message = sprintf( __( 'A new queue has begun processing. <a href="%s">View actions in-progress &raquo;</a>', 'pronamic-pay-with-mollie-for-woocommerce' ), esc_url( $in_progress_url ) );
			} else {
				/* translators: %d: seconds */
				$async_request_message = sprintf( __( 'The next queue will begin processing in approximately %d seconds.', 'pronamic-pay-with-mollie-for-woocommerce' ), $async_request_lock_expiration - time() );
			}
			...
			if ( 1 == $notification['success'] ) {
				...
				switch ( $notification['row_action_type'] ) {
					case 'run' :
						...
						$action_message_html = sprintf( __( 'Successfully executed action: %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_hook_html );
						break;
					case 'cancel' :
						/* translators: %s: action HTML */
						$action_message_html = sprintf( __( 'Successfully canceled action: %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_hook_html );
						break;
					default :
						/* translators: %s: action HTML */
						$action_message_html = sprintf( __( 'Successfully processed change for action: %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_hook_html );
						break;
				}
			} else {
				$class = 'error';
				/* translators: 1: action HTML 2: action ID 3: error message */
				$action_message_html = sprintf( __( 'Could not process change for action: "%1$s" (ID: %2$d). Error: %3$s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_hook_html, esc_html( $notification['action_id'] ), esc_html( $notification['error_message'] ) );
			}
	...
	protected function get_schedule_display_string( ActionScheduler_Schedule $schedule ) {
		...
		if ( is_a( $schedule, 'ActionScheduler_NullSchedule' ) ) {
			return __( 'async', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
		...
		if ( gmdate( 'U' ) > $next_timestamp ) {
			...
			$schedule_display_string .= sprintf( __( ' (%s ago)', 'pronamic-pay-with-mollie-for-woocommerce' ), self::human_interval( gmdate( 'U' ) - $next_timestamp ) );
		} else {
			/* translators: %s: date interval */
			$schedule_display_string .= sprintf( __( ' (%s)', 'pronamic-pay-with-mollie-for-woocommerce' ), self::human_interval( $next_timestamp - gmdate( 'U' ) ) );
		}
			...
			try {
				...
				error_log(
					sprintf(
						...
						__( 'Action Scheduler was unable to delete action %1$d. Reason: %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
						$id,
						$e->getMessage()
					)
				);
	...
	protected function get_search_box_button_text() {
		return __( 'Search hook, args and claim ID', 'pronamic-pay-with-mollie-for-woocommerce' );
	}
