...
class ActionScheduler_wpPostStore extends ActionScheduler_Store {
	...
	protected function save_post_array( $post_array ) {
		...
		if ( is_wp_error( $post_id ) || empty( $post_id ) ) {
			throw new RuntimeException( __( 'Unable to save action.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		}
		return $post_id;
	}
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( __( 'Invalid schedule. Cannot save action.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		}
	...
	protected function claim_actions( $claim_id, $limit, DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			throw new RuntimeException( __( 'Unable to claim actions. Database error.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		}
	...
	protected function get_actions_by_group( $group, $limit, DateTime $date ) {
		...
		if ( ! term_exists( $group, self::GROUP_TAXONOMY ) ) {
			...
			throw new InvalidArgumentException( sprintf( __( 'The group "%s" does not exist.', 'pronamic-pay-with-mollie-for-woocommerce' ), $group ) );
		}
	...
	public function release_claim( ActionScheduler_ActionClaim $claim ) {
		...
		if ( false === $result ) {
			...
			throw new RuntimeException( sprintf( __( 'Unable to unlock claim %s. Database error.', 'pronamic-pay-with-mollie-for-woocommerce' ), $claim->get_id() ) );
		}
	}
	...
	public function unclaim_action( $action_id ) {
		...
		if ( false === $result ) {
			...
			throw new RuntimeException( sprintf( __( 'Unable to unlock claim on action %s. Database error.', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id ) );
		}
	}
	...
	public function mark_failure( $action_id ) {
		...
		if ( false === $result ) {
			/* translators: %s: action ID */
			throw new RuntimeException( sprintf( __( 'Unable to mark failure on action %s. Database error.', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id ) );
		}
	}
	...
	protected function validate_action( ActionScheduler_Action $action ) {
		try {
			...
			$message = sprintf( __( '%s Support for strings longer than this will be removed in a future version.', 'pronamic-pay-with-mollie-for-woocommerce' ), $e->getMessage() );
