	...
	private function save_action_to_db( ActionScheduler_Action $action, DateTime $date = null, $unique = false ) {
		...
		try {
			...
			if ( is_wp_error( $action_id ) ) {
				...
				throw new \RuntimeException( $wpdb->last_error ? $wpdb->last_error : __( 'Database error.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
			}
			...
			throw new \RuntimeException( sprintf( __( 'Error saving action: %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $e->getMessage() ), 0 );
		}
	}
...
",
	...
	protected function get_query_actions_sql( array $query, $select_or_count = 'select' ) {
		...
		if ( ! in_array( $select_or_count, array( 'select', 'count' ), true ) ) {
			throw new InvalidArgumentException( __( 'Invalid value for select or count parameter. Cannot query actions.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		}
		...
		if ( ! is_null( $query['args'] ) ) {
			switch ( $query['partial_args_matching'] ) {
				case 'json':
					if ( ! $supports_json ) {
						throw new \RuntimeException( __( 'JSON partial matching not supported in your environment. Please check your MySQL/MariaDB version.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
					}
					...
					foreach ( $query['args'] as $key => $value ) {
						...
						if ( ! $placeholder ) {
							throw new \RuntimeException( sprintf(
								...
								__( 'The value type for the JSON partial matching is not supported. Must be either integer, boolean, double or string. %s type provided.', 'pronamic-pay-with-mollie-for-woocommerce' ),
								$value_type
							) );
						}
				...
				case 'off':
					...
					break;
				default:
					throw new \RuntimeException( __( 'Unknown partial args matching value.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
			}
		}
	...
	public function cancel_action( $action_id ) {
		...
		if ( false === $updated ) {
			...
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id ) );
		}
	...
	public function delete_action( $action_id ) {
		...
		if ( empty( $deleted ) ) {
			throw new \InvalidArgumentException( sprintf( __( 'Unidentified action %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id ) ); //phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		}
	...
	protected function claim_actions( $claim_id, $limit, \DateTime $before_date = null, $hooks = array(), $group = '' ) {
		...
		if ( false === $rows_affected ) {
			$error = empty( $wpdb->last_error )
				? _x( 'unknown', 'database error', 'pronamic-pay-with-mollie-for-woocommerce' )
				: $wpdb->last_error;

			throw new \RuntimeException(
				sprintf(
					/* translators: %s database error. */
					__( 'Unable to claim actions. Database error: %s.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$error
				)
			);
		}
		...
		if ( $row_updates < count( $action_ids ) ) {
			throw new RuntimeException(
				sprintf(
					...
					__( 'Unable to release actions from claim id %d.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$claim->get_id()
				)
			);
		}
		...
		if ( ! $status_updated ) {
			throw new Exception(
				sprintf(
					...
					__( 'Unable to update the status of action %1$d to %2$s.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$action_id,
					self::STATUS_RUNNING
				)
			);
	...
	public function get_status( $action_id ) {
		...
		if ( null === $status ) {
			throw new \InvalidArgumentException( __( 'Invalid action ID. No status found.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		} elseif ( empty( $status ) ) {
			throw new \RuntimeException( __( 'Unknown status found for action.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		} else {
			return $status;
		}
	}
