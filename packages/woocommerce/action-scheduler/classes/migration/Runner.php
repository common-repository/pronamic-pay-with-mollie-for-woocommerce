...
class Runner {
	...
	public function run( $batch_size = 10 ) {
		...
		if ( $this->progress_bar ) {
			...
			$this->progress_bar->set_message( sprintf( _n( 'Migrating %d action', 'Migrating %d actions', $batch_size, 'pronamic-pay-with-mollie-for-woocommerce' ), $batch_size ) );
		...
		foreach ( $action_ids as $source_action_id ) {
			...
			if ( $destination_action_id ) {
				$this->destination_logger->log( $destination_action_id, sprintf(
					...
					__( 'Migrated action with ID %1$d in %2$s to ID %3$d in %4$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$source_action_id,
