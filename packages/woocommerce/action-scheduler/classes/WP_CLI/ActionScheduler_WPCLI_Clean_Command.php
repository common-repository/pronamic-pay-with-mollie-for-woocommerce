	...
	protected function print_total_batches( int $batches_processed ) {
		WP_CLI::log(
			sprintf(
				...
				_n( '%d batch processed.', '%d batches processed.', $batches_processed, 'pronamic-pay-with-mollie-for-woocommerce' ),
				$batches_processed
			)
		);
	}
	...
	protected function print_error( Exception $e ) {
		WP_CLI::error(
			sprintf(
				...
				__( 'There was an error deleting an action: %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
				$e->getMessage()
			)
		);
	}
	...
	protected function print_success( int $actions_deleted ) {
		WP_CLI::success(
			sprintf(
				...
				_n( '%d action deleted.', '%d actions deleted.', $actions_deleted, 'pronamic-pay-with-mollie-for-woocommerce' ),
				$actions_deleted
			)
		);
	}
