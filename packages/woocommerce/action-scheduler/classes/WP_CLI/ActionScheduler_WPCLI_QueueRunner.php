...
class ActionScheduler_WPCLI_QueueRunner extends ActionScheduler_Abstract_QueueRunner {
	...
	public function __construct( ActionScheduler_Store $store = null, ActionScheduler_FatalErrorMonitor $monitor = null, ActionScheduler_QueueCleaner $cleaner = null ) {
		if ( ! ( defined( 'WP_CLI' ) && WP_CLI ) ) {
			...
			throw new Exception( sprintf( __( 'The %s class can only be run within WP CLI.', 'pronamic-pay-with-mollie-for-woocommerce' ), __CLASS__ ) );
		}
	...
	public function setup( $batch_size, $hooks = array(), $group = '', $force = false ) {
		...
		if ( $this->has_maximum_concurrent_batches() ) {
			if ( $force ) {
				WP_CLI::warning( __( 'There are too many concurrent batches, but the run is forced to continue.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
			} else {
				WP_CLI::error( __( 'There are too many concurrent batches.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
			}
		}
	...
	protected function setup_progress_bar() {
		...
		$this->progress_bar = new ProgressBar(
			...
			sprintf( _n( 'Running %d action', 'Running %d actions', $count, 'pronamic-pay-with-mollie-for-woocommerce' ), $count ),
			$count
		);
	}
	...
	public function run( $context = 'WP CLI' ) {
		...
		foreach ( $this->actions as $action_id ) {
			...
			if ( ! in_array( $action_id, $this->store->find_actions_by_claim_id( $this->claim->get_id() ) ) ) {
				WP_CLI::warning( __( 'The claim has been lost. Aborting current batch.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
				break;
			}
	...
	public function before_execute( $action_id ) {
		...
		WP_CLI::log( sprintf( __( 'Started processing action %s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id ) );
	}
	...
	public function after_execute( $action_id, $action = null ) {
		...
		WP_CLI::log( sprintf( __( 'Completed processing action %1$s with hook: %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id, $action->get_hook() ) );
	}
	...
	public function action_failed( $action_id, $exception ) {
		WP_CLI::error(
			...
			sprintf( __( 'Error processing action %1$s: %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ), $action_id, $exception->getMessage() ),
			false
		);
	}
