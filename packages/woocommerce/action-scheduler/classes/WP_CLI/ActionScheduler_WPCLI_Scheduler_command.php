			...
			if ( is_subclass_of( $classname, ActionScheduler_Abstract_Schema::class ) ) {
				...
				WP_CLI::success(
					sprintf(
						...
						__( 'Registered schema for %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
						$classname
					)
				);
			}
	...
	protected function print_total_actions( $total ) {
		WP_CLI::log(
			sprintf(
				...
				_n( 'Found %d scheduled task', 'Found %d scheduled tasks', $total, 'pronamic-pay-with-mollie-for-woocommerce' ),
				$total
			)
		);
	}
	...
	protected function print_total_batches( $batches_completed ) {
		WP_CLI::log(
			sprintf(
				...
				_n( '%d batch executed.', '%d batches executed.', $batches_completed, 'pronamic-pay-with-mollie-for-woocommerce' ),
				$batches_completed
			)
		);
	}
	...
	protected function print_error( Exception $e ) {
		WP_CLI::error(
			sprintf(
				...
				__( 'There was an error running the action scheduler: %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
				$e->getMessage()
			)
		);
	}
	...
	protected function print_success( $actions_completed ) {
		WP_CLI::success(
			sprintf(
				...
				_n( '%d scheduled task completed.', '%d scheduled tasks completed.', $actions_completed, 'pronamic-pay-with-mollie-for-woocommerce' ),
				$actions_completed
			)
		);
	}
