...
class ActionScheduler_ActionFactory {
	...
	public function repeat( $action ) {
		...
		if ( is_null( $next ) || ! $schedule->is_recurring() ) {
			throw new InvalidArgumentException( __( 'Invalid action - must be a recurring action.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		}
		...
		try {
			...
			error_log(
				sprintf(
					...
					__( 'Caught exception while enqueuing action "%1$s": %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$options['hook'],
					$e->getMessage()
				)
			);
