...
class ActionScheduler_WPCommentCleaner {
	...
	public static function print_admin_notice() {
		...
		if ( $next_scheduled_cleanup_hook ) {
			...
			$next_cleanup_message = sprintf( __( 'This data will be deleted in %s.', 'pronamic-pay-with-mollie-for-woocommerce' ), human_time_diff( gmdate( 'U' ), $next_scheduled_cleanup_hook ) );
		}

		$notice = sprintf(
			/* translators: 1: next cleanup message 2: github issue URL */
			__( 'Action Scheduler has migrated data to custom tables; however, orphaned log entries exist in the WordPress Comments table. %1$s <a href="%2$s">Learn more &raquo;</a>', 'pronamic-pay-with-mollie-for-woocommerce' ),
			$next_cleanup_message,
