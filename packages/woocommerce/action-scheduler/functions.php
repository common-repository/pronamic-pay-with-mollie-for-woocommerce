	...
	if ( ! is_numeric( $interval_in_seconds ) || $interval_in_seconds != $interval ) {
		_doing_it_wrong(
			...
			sprintf(
				...
				esc_html__( 'An integer was expected but "%1$s" (%2$s) was received.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		try {
			...
			ActionScheduler::logger()->log(
				...
				sprintf(
					...
					__( 'Caught exception while cancelling action "%1$s": %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$hook,
