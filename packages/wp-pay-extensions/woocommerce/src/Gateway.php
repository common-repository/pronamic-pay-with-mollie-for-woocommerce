...
class Gateway extends WC_Payment_Gateway {
	...
	public function __construct( $args = [] ) {
		...
		if ( null === $this->method_title ) {
			$this->method_title = sprintf(
				...
				__( '%1$s - %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
				__( 'Pronamic', 'pronamic-pay-with-mollie-for-woocommerce' ),
				PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-pay-with-mollie-for-woocommerce' ) )
			);
		}
		...
		if ( null !== $this->payment_method ) {
			$this->order_button_text = sprintf(
				...
				__( 'Proceed to %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
				PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-pay-with-mollie-for-woocommerce' ) )
			);
		}
	...
	public function init_form_fields() {
		...
		$this->form_fields = [
			'enabled'             => [
				'title'   => __( 'Enable/Disable', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'    => 'checkbox',
				'label'   => sprintf(
					/* translators: %s: payment method title */
					__( 'Enable %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$this->method_title
				),
				'default' => 'no',
			],
			'title'               => [
				'title'       => __( 'Title', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'        => 'text',
				'description' => $description_prefix . __( 'This controls the title which the user sees during checkout.', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'default'     => PaymentMethods::get_name( $this->payment_method, __( 'Pronamic', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
			],
			'description'         => [
				'title'       => __( 'Description', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'        => 'textarea',
				'description' => $description_prefix . sprintf(
					/* translators: %s: payment method title */
					__( 'Give the customer instructions for paying via %s, and let them know that their order won\'t be shipping until the money is received.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$this->method_title
				),
				'default'     => '',
			],
			'icon_display'        => [
				'title'       => __( 'Icon display', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'        => 'select',
				'options'     => [
					'default' => \__( 'Default', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'none'    => \__( 'None', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'custom'  => \__( 'Custom', 'pronamic-pay-with-mollie-for-woocommerce' ),
				],
				'description' => sprintf(
					'%s%s',
					$description_prefix,
					__( 'This controls how the payment gateway icon is displayed on the checkout page.', 'pronamic-pay-with-mollie-for-woocommerce' )
				),
				'default'     => ( '' === $icon_display && '' !== $icon_url ) ? 'custom' : '',
			],
			'icon'                => [
				'title'       => __( 'Custom icon URL', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'        => 'text',
				'description' => sprintf(
					'%s%s',
					$description_prefix,
					__( 'Provide a publicly accessible URL for your own payment gateway icon (only used if "Custom" is selected in the icon display setting).', 'pronamic-pay-with-mollie-for-woocommerce' )
				),
				'default'     => '',
			],
			'config_id'           => [
				'title'   => __( 'Configuration', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'    => 'select',
				'default' => get_option( 'pronamic_pay_config_id' ),
				'options' => Plugin::get_config_select_options( $this->payment_method ),
			],
			'payment'             => [
				'title'       => __( 'Payment Options', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'        => 'title',
				'description' => '',
				'default'     => '',
			],
			'payment_description' => [
				'title'       => __( 'Payment Description', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'type'        => 'text',
				'description' => sprintf(
					'%s%s<br />%s<br />%s',
					$description_prefix,
					__( 'This controls the payment description.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					/* translators: %s: default code */
					sprintf( __( 'Default: <code>%s</code>', 'pronamic-pay-with-mollie-for-woocommerce' ), __( 'Order {order_number}', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
					/* translators: %s: tags */
					sprintf( __( 'Tags: %s', 'pronamic-pay-with-mollie-for-woocommerce' ), sprintf( '<code>%s</code> <code>%s</code> <code>%s</code>', '{order_number}', '{order_date}', '{payment_lines_name}', '{blogname}' ) )
				),
				'default'     => __( 'Order {order_number}', 'pronamic-pay-with-mollie-for-woocommerce' ),
			],
		];
		...
		if ( isset( $this->gateway_args['icon'] ) ) {
			...
			$this->form_fields['icon']['description'] = sprintf(
				'%s%s<br />%s',
				$description_prefix,
				__( 'This controls the icon which the user sees during checkout.', 'pronamic-pay-with-mollie-for-woocommerce' ),
				/* translators: %s: default code */
				sprintf( __( 'Default: <code>%s</code>', 'pronamic-pay-with-mollie-for-woocommerce' ), $this->form_fields['icon']['default'] )
			);
		}
	...
	public function process_payment( $order_id ) {
		...
		if ( null === $gateway ) {
			$notice = __( 'The payment gateway could not be found.', 'pronamic-pay-with-mollie-for-woocommerce' );
			...
			if ( current_user_can( 'manage_options' ) && empty( $this->config_id ) ) {
				...
				$notice = sprintf(
					...
					__( 'You have to select a gateway configuration on the <a href="%s">WooCommerce checkout settings page</a>.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					add_query_arg(
						[
							'page'    => 'wc-settings',
							'tab'     => 'checkout',
	...
	private function new_pronamic_payment_from_wc_order( WC_Order $order ) {
		...
		$title = sprintf(
			...
			__( 'Payment for %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
			sprintf(
				/* translators: %s: order id */
				__( 'WooCommerce order %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( null === $gateway ) {
			return new \WP_Error(
				...
				__( 'Unable to process refund as gateway configuration does not exist.', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
		}
		...
		if ( null === $payment ) {
			return new \WP_Error(
				...
				\__( 'Cannot process refund because payment could not be found.', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
		}
		...
		try {
			...
			$note = \sprintf(
				...
				\__( 'Created refund of %1$s with reference `%2$s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function after_checkout_validation( $data, $errors ) {
		...
		foreach ( $input_ids as $input_id ) {
			...
			if ( empty( $input_value ) ) {
				$error = sprintf(
					/* translators: %s: payment method title */
					__( 'A required field for the %s payment method is empty.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$this->method_title
				);
