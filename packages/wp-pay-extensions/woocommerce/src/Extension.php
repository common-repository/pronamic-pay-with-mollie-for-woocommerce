...
class Extension extends AbstractPluginIntegration {
	...
	public function __construct( $args = [] ) {
		$args = wp_parse_args(
			$args,
			[
				'name'                => \__( 'WooCommerce', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public static function get_gateways() {
		...
		$gateways = [
			[
				'id'                 => 'pronamic_pay',
				'payment_method'     => null,
				'method_title'       => __( 'Pronamic', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'method_description' => __( "This payment method does not use a predefined payment method for the payment. Some payment providers list all activated payment methods for your account to choose from. Use payment method specific gateways (such as 'iDEAL') to let customers choose their desired payment method at checkout.", 'pronamic-pay-with-mollie-for-woocommerce' ),
				'check_active'       => false,
			],
		];
	...
	public static function woocommerce_thankyou_order_received_text( $message, $order ) {
		...
		// Add notice.
		$message .= ' ' . \__( 'We process your order as soon as we have processed your payment.', 'pronamic-pay-with-mollie-for-woocommerce' );
	...
	public static function reservation_cancelled_note( Payment $payment ) {
		...
		$order->add_order_note(
			sprintf(
				...
				__( 'reserved payment cancelled', 'pronamic-pay-with-mollie-for-woocommerce' )
			)
		);
	}
	...
	public static function status_update( Payment $payment ) {
		...
		$note = sprintf(
			...
			__( '<a href="%1$s">Payment #%2$s</a> via "%3$s" updated to "%4$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			if ( \is_wp_error( $result ) ) {
				...
				$payment->add_note(
					\sprintf(
						...
						\__( 'Unable to create WooCommerce refund for "%1$s", due to the following error: "%2$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
						$refund->psp_id,
						$error_message
					)
				);
	...
	public static function admin_init() {
		...
		add_settings_section(
			...
			__( 'WooCommerce', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		add_settings_field(
			'pronamic_pay_woocommerce_birth_date_field',
			__( 'Date of birth checkout field', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		add_settings_field(
			'pronamic_pay_woocommerce_birth_date_field_enable',
			__( 'Add date of birth field', 'pronamic-pay-with-mollie-for-woocommerce' ),
			[ __CLASS__, 'input_checkbox' ],
			'pronamic_pay',
			'pronamic_pay_woocommerce',
			[
				'legend'      => __( 'Add date of birth field', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'description' => __( 'Add date of birth field to billing checkout fields', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		add_settings_field(
			'pronamic_pay_woocommerce_gender_field',
			__( 'Gender checkout field', 'pronamic-pay-with-mollie-for-woocommerce' ),
			[ __CLASS__, 'input_checkout_fields_select' ],
			'pronamic_pay',
			'pronamic_pay_woocommerce',
			[
		...
		add_settings_field(
			'pronamic_pay_woocommerce_gender_field_enable',
			__( 'Add gender field', 'pronamic-pay-with-mollie-for-woocommerce' ),
			[ __CLASS__, 'input_checkbox' ],
			'pronamic_pay',
			'pronamic_pay_woocommerce',
			[
				'legend'      => __( 'Add gender field', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'description' => __( 'Add gender field to billing checkout fields', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public static function input_checkout_fields_select( $args ) {
		...
		$placeholder_option->children[] = \__( '— Select a checkout field —', 'pronamic-pay-with-mollie-for-woocommerce' );
	...
	public static function checkout_fields( $fields ) {
		...
		if ( $enable_birth_date_field ) {
			$fields['billing']['pronamic_pay_birth_date'] = [
				'type'     => 'date',
				'label'    => __( 'Date of birth', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'priority' => 110,
			];
		}
		...
		if ( $enable_gender_field ) {
			$fields['billing']['pronamic_pay_gender'] = [
				'type'     => 'select',
				'label'    => __( 'Gender', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'priority' => 120,
				'options'  => [
					''  => __( '— Select gender —', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'F' => __( 'Female', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'M' => __( 'Male', 'pronamic-pay-with-mollie-for-woocommerce' ),
					'X' => __( 'Other', 'pronamic-pay-with-mollie-for-woocommerce' ),
				],
			];
		}
	...
	public static function source_text( $text, Payment $payment ) {
		...
		$order_edit_link = \sprintf(
			...
			\__( 'Order %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
			$source_id
		);
	...
	public static function source_description( $description, Payment $payment ) {
		return __( 'WooCommerce Order', 'pronamic-pay-with-mollie-for-woocommerce' );
	}
