...
if ( ! $integration || ! $integration->supports( 'webhook_log' ) ) {
	esc_html_e( 'This gateway does not support webhook logging.', 'pronamic-pay-with-mollie-for-woocommerce' );
...
if ( empty( $webhook_log_json_string ) || $config_gateway_id !== $gateway_id ) {
	esc_html_e( 'No webhook request processed yet.', 'pronamic-pay-with-mollie-for-woocommerce' );
...
try {
	...
	printf(
		...
		esc_html__( 'The following error occurred when reading the webhook request information: "%s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
...
if ( null !== $payment_id ) {
	echo wp_kses(
		sprintf(
			...
			$webhook_log_request_info->get_request_date()->format_i18n( _x( 'l j F Y \a\t H:i', 'full datetime format', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
