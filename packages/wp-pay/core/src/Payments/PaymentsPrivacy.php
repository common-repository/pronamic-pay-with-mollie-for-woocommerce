...
class PaymentsPrivacy {
		...
		foreach ( $payments as $payment ) {
			...
			if ( ! empty( $export_data ) ) {
				$items[] = [
					...
					'group_label' => __( 'Payments', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function payments_anonymizer( $email_address ) {
		...
		foreach ( $payments as $payment ) {
			...
			// Add payment note.
			$payment->add_note( __( 'Payment anonymized for personal data erasure request.', 'pronamic-pay-with-mollie-for-woocommerce' ) );

			// Add message.
			/* translators: %s: Payment ID */
			$messages[] = sprintf( __( 'Payment ID %s anonymized.', 'pronamic-pay-with-mollie-for-woocommerce' ), $payment_id );
