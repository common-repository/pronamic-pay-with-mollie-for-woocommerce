	...
	private function get_payment_status_update_note( $old_status, $new_status ) {
		...
		if ( null === $old_status ) {
			return sprintf(
				...
				__( 'Payment created with status "%1$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		return sprintf(
			...
			__( 'Payment status changed from "%1$s" to "%2$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		if ( null === $payment ) {
			return new \WP_Error(
				...
				\sprintf(
					...
					\__( 'Could not find payment with ID `%s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$payment_id
				),
				$payment_id
			);
