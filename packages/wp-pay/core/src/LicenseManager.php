...
class LicenseManager {
	...
	public function admin_init() {
		...
		\add_settings_field(
			...
			\__( 'Support License Key', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		if ( 'valid' !== get_option( 'pronamic_pay_license_status' ) ) {
			...
			if ( '' === $license ) {
				...
				/* translators: 1: Pronamic Pay settings page URL, 2: The pronamicpay.com plugin page URL */
					__( '<strong>Pronamic Pay</strong> — You have not entered a valid <a href="%1$s">support license key</a>, please <a href="%2$s" target="_blank">get your key at pronamicpay.com</a>.', 'pronamic-pay-with-mollie-for-woocommerce' ),
				...
				/* translators: 1: Pronamic Pay settings page URL, 2: The pronamicpay.com plugin page URL, 3: The pronamic.shop account page URL */
					__( '<strong>Pronamic Pay</strong> — You have not entered a valid <a href="%1$s">support license key</a>. Please <a href="%2$s" target="_blank">get your key at pronamicpay.com</a> or login to <a href="%3$s" target="_blank">check your license status</a>.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					add_query_arg( 'page', 'pronamic_pay_settings', get_admin_url( null, 'admin.php' ) ),
					'https://www.pronamicpay.com/',
	...
	public function get_formatted_license_status() {
		...
		switch ( $license_status ) {
			case 'valid':
				return __( 'Valid', 'pronamic-pay-with-mollie-for-woocommerce' );

			case 'invalid':
				return __( 'Invalid', 'pronamic-pay-with-mollie-for-woocommerce' );

			case 'site_inactive':
				return __( 'Site Inactive', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
	...
	public function get_formatted_next_license_check() {
		$next_license_check = esc_html__( 'Not scheduled', 'pronamic-pay-with-mollie-for-woocommerce' );
	...
	public function debug_information( $debug_information ) {
		...
		$fields = [
			// License key.
			'license_key'        => [
				'label'   => __( 'Support license key', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'value'   => esc_html( get_option( 'pronamic_pay_license_key', __( 'No license key found', 'pronamic-pay-with-mollie-for-woocommerce' ) ) ),
				'private' => true,
			],

			// License status.
			'license_status'     => [
				'label' => __( 'License status', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'value' => esc_html( $this->get_formatted_license_status() ),
			],

			// Next scheduled license check.
			'next_license_check' => [
				'label' => __( 'Next scheduled license check', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function site_status_tests( $status_tests ) {
		...
		$status_tests['direct']['pronamic_pay_valid_license'] = [
			'label' => \__( 'Pronamic Pay support license key test', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function test_valid_license() {
		...
		$result = [
			...
			'label'       => \__( 'Pronamic Pay license key is valid', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'description' => \sprintf(
				'<p>%s</p>',
				\__( 'A valid license is required for technical support and continued plugin updates.', 'pronamic-pay-with-mollie-for-woocommerce' )
			),
			'badge'       => [
				'label' => \__( 'Security', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'color' => 'blue',
			],
			'status'      => 'good',
			'actions'     => '',
		...
		if ( 'valid' !== \get_option( 'pronamic_pay_license_status' ) ) {
			...
			$result['label']  = \__( 'No valid license key for Pronamic Pay', 'pronamic-pay-with-mollie-for-woocommerce' );
			...
			if ( '' === \get_option( 'pronamic_pay_license_key' ) ) {
				$result['actions'] .= \sprintf(
					...
					\__( 'Purchase license', 'pronamic-pay-with-mollie-for-woocommerce' )
				);
			}
			...
			$result['actions'] .= \sprintf(
				'<a href="%s">%s</a> - ',
				\add_query_arg( 'page', 'pronamic_pay_settings', \get_admin_url( null, 'admin.php' ) ),
				\__( 'License settings', 'pronamic-pay-with-mollie-for-woocommerce' )
			);

			$result['actions'] .= \sprintf(
				'<a href="%s">%s</a>',
				\esc_url( 'https://www.pronamic.eu/account/' ),
				\__( 'Check existing license', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
