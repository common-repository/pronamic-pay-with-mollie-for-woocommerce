...
class SubscriptionsPrivacy {
	...
	public function register_exporters( $privacy_manager ) {
		...
		$privacy_manager->add_exporter(
			'subscriptions',
			__( 'Subscriptions', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		foreach ( $subscriptions as $subscription ) {
			...
			if ( ! empty( $export_data ) ) {
				$items[] = [
					...
					'group_label' => __( 'Subscriptions', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function subscriptions_anonymizer( $email_address ) {
		...
		foreach ( $subscriptions as $subscription ) {
			...
			$note = __( 'Subscription anonymized for personal data erasure request.', 'pronamic-pay-with-mollie-for-woocommerce' );
			/* translators: %s = subscription id */
			$message = __( 'Subscription ID %s anonymized.', 'pronamic-pay-with-mollie-for-woocommerce' );
			...
			if ( isset( $subscription_status ) && in_array( $subscription_status, [ SubscriptionStatus::COMPLETED, SubscriptionStatus::CANCELLED ], true ) ) {
				...
				$note = __( 'Subscription not anonymized for personal data erasure request because of active status.', 'pronamic-pay-with-mollie-for-woocommerce' );

				/* translators: %s: Subscription ID */
				$message = __( 'Subscription ID %s not anonymized because of active status.', 'pronamic-pay-with-mollie-for-woocommerce' );
