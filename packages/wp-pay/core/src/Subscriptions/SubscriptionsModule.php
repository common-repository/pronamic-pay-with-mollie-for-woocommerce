...
class SubscriptionsModule {
	...
	private function get_subscription_status_update_note( $old_status, $new_status ) {
		...
		if ( null === $old_status ) {
			...
			/* translators: 1: new status */
				__( 'Subscription created with status "%1$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		/* translators: 1: old status, 2: new status */
			__( 'Subscription status changed from "%1$s" to "%2$s".', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			if ( ! empty( $mandate_id ) ) {
				try {
					if ( ! \is_callable( [ $gateway, 'update_subscription_mandate' ] ) ) {
						throw new \Exception( __( 'Gateway does not support subscription mandate updates.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
					}
	...
	public function rest_api_init() {
		\register_rest_route(
			...
			[
				...
				'args'                => [
					'subscription_id' => [
						'description' => __( 'Subscription ID.', 'pronamic-pay-with-mollie-for-woocommerce' ),
						'type'        => 'integer',
					],
				],
			]
		...
		\register_rest_route(
			...
			[
				...
				'args'                => [
					...
					'sequence_number' => [
						'description' => __( 'Subscription phase sequence number.', 'pronamic-pay-with-mollie-for-woocommerce' ),
						'type'        => 'integer',
					],
				],
			]
		...
		if ( null === $subscription ) {
			return new \WP_Error(
				...
				\sprintf(
					...
					\__( 'Could not find subscription with ID `%s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$subscription_id
				),
				$subscription_id
			);
	...
	public function rest_api_subscription_phase( \WP_REST_Request $request ) {
		...
		if ( null === $phase ) {
			return new \WP_Error(
				...
				\sprintf(
					/* translators: %s: Subscription ID */
					\__( 'Could not find subscription phase with sequence number `%s`.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$sequence_number
				),
				$sequence_number
			);
	...
	public function source_text_subscription_payment_method_change( $text ) {
		$text = \__( 'Subscription payment method change', 'pronamic-pay-with-mollie-for-woocommerce' );
