...
class AdminSubscriptionPostType {
	...
	public function admin_notices() {
		...
		foreach ( $payment_ids as $payment_id ) {
			$edit_post_link = \sprintf(
				...
				__( 'Payment #%d', 'pronamic-pay-with-mollie-for-woocommerce' ),
				$payment_id
			);
			...
			\printf(
				...
				\wp_kses_post(
					\sprintf(
						...
						__( '%s has been created.', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function columns( $columns ) {
		$columns = [
			...
			'pronamic_subscription_title'        => __( 'Subscription', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_subscription_customer'     => __( 'Customer', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_subscription_amount'       => __( 'Amount', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_subscription_recurring'    => __( 'Recurrence', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_subscription_next_payment' => __( 'Next payment', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_subscription_date'         => __( 'Date', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
	...
	public function custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'pronamic_subscription_next_payment':
				...
				echo empty( $next_payment_date ) ? '—' : esc_html( $next_payment_date->format_i18n( \__( 'D j M Y', 'pronamic-pay-with-mollie-for-woocommerce' ) ) );
	...
	public function add_meta_boxes( $post_type ) {
		...
		add_meta_box(
			...
			__( 'Phases', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		add_meta_box(
			...
			__( 'Payments', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function post_updated_messages( $messages ) {
		...
		$messages[ self::POST_TYPE ] = [
			...
			1  => __( 'Subscription updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			4  => __( 'Subscription updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Subscription restored to revision from %s.', 'pronamic-pay-with-mollie-for-woocommerce' ), strval( wp_post_revision_title( (int) $_GET['revision'], false ) ) ) : false,
			...
			6  => __( 'Subscription published.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			7  => __( 'Subscription saved.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			8  => __( 'Subscription submitted.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			9  => sprintf( __( 'Subscription scheduled for: %s.', 'pronamic-pay-with-mollie-for-woocommerce' ), '<strong>' . $scheduled_date . '</strong>' ),
			...
			10 => __( 'Subscription draft updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
			...
			if ( ! empty( $new_value ) && $old_value !== $new_value ) {
				...
				if ( false !== $updated_date ) {
					...
					$note = \sprintf(
						...
						\__( 'Next payment date updated from %1$s to %2$s.', 'pronamic-pay-with-mollie-for-woocommerce' ),
						null === $next_payment_date ? '' : $next_payment_date->format_i18n( \__( 'D j M Y', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
						$updated_date->format_i18n( \__( 'D j M Y', 'pronamic-pay-with-mollie-for-woocommerce' ) )
					);
