	...
	private function get_navigation( $current ) {
		...
		if ( false !== $previous_url ) {
			$content .= \sprintf(
				...
				\esc_html__( 'Previous', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
		...
		if ( false !== $next_url ) {
			$content .= \sprintf(
				'<a href="%s" class="button-primary pp-pointer-button-next">%s</a>',
				\esc_url( $next_url ),
				\esc_html__( 'Next', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
		...
		$content .= \sprintf(
			...
			\esc_html__( 'Close', 'pronamic-pay-with-mollie-for-woocommerce' )
		);
