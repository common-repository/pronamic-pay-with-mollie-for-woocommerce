...
class AdminPaymentPostType {
	...
	public function maybe_process_payment_action() {
		...
		if ( filter_has_var( INPUT_GET, 'pronamic_pay_check_status' ) && check_admin_referer( 'pronamic_payment_check_status_' . $post_id ) ) {
			...
			$this->admin_notices[] = [
				'type'    => 'info',
				'message' => __( 'Payment status updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			];
		}
	}
	...
	public function maybe_display_anonymized_notice() {
		...
		$this->admin_notices[] = [
			'type'    => 'info',
			'message' => __( 'This payment has been anonymized. Personal details are not available anymore.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
	}
	...
	public function columns( $columns ) {
		$columns = [
			...
			'pronamic_payment_status'       => sprintf(
				...
				esc_html__( 'Status', 'pronamic-pay-with-mollie-for-woocommerce' ),
				esc_html__( 'Status', 'pronamic-pay-with-mollie-for-woocommerce' )
			),
			'pronamic_payment_subscription' => sprintf(
				...
				esc_html__( 'Subscription', 'pronamic-pay-with-mollie-for-woocommerce' ),
				esc_html__( 'Subscription', 'pronamic-pay-with-mollie-for-woocommerce' )
			),
			'pronamic_payment_method'       => '',
			'pronamic_payment_title'        => __( 'Payment', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_payment_transaction'  => __( 'Transaction', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_payment_gateway'      => __( 'Gateway', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_payment_description'  => __( 'Description', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_payment_customer'     => __( 'Customer', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_payment_amount'       => __( 'Amount', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'pronamic_payment_date'         => __( 'Date', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
	...
	public function custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'pronamic_payment_status':
				...
				$label = __( 'Unknown', 'pronamic-pay-with-mollie-for-woocommerce' );
			...
			case 'pronamic_payment_subscription':
				...
				foreach ( $subscriptions as $subscription ) {
					$label = __( 'Recurring payment', 'pronamic-pay-with-mollie-for-woocommerce' );
					$class = 'pronamic-pay-icon-recurring';

					if ( $subscription->is_first_payment( $payment ) ) {
						$label = __( 'First of recurring payment', 'pronamic-pay-with-mollie-for-woocommerce' );
			...
			case 'pronamic_payment_title':
				...
				echo wp_kses(
					sprintf(
						...
						__( '%1$s for %2$s %3$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
						$text,
						$source_description,
						$source_id_text
					),
			...
			case 'pronamic_payment_amount':
				...
				if ( ! $refunded_amount->is_zero() ) {
					...
					$tip[] = \sprintf(
						...
						__( '%s refunded', 'pronamic-pay-with-mollie-for-woocommerce' ),
				...
				if ( null !== $charged_back_amount ) {
					...
					$tip[] = \sprintf(
						/* translators: %s: formatted amount */
						__( '%s charged back', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function add_meta_boxes( $post_type ) {
		...
		add_meta_box(
			'pronamic_payment',
			__( 'Payment', 'pronamic-pay-with-mollie-for-woocommerce' ),
			[ $this, 'meta_box_info' ],
			$post_type,
			'normal',
			'high'
		...
		add_meta_box(
			...
			__( 'Payment Lines', 'pronamic-pay-with-mollie-for-woocommerce' ),
			[ $this, 'meta_box_lines' ],
			$post_type,
			'normal',
			'high'
		...
		if ( \in_array( 'subscriptions', $modules, true ) ) {
			\add_meta_box(
				...
				\__( 'Subscription', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		\add_meta_box(
			...
			\__( 'Refunds', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		add_meta_box(
			...
			__( 'Notes', 'pronamic-pay-with-mollie-for-woocommerce' ),
			[ $this, 'meta_box_notes' ],
			$post_type,
			'normal',
			'high'
	...
	public function post_updated_messages( $messages ) {
		...
		$scheduled_date = date_i18n( __( 'M j, Y @ H:i', 'pronamic-pay-with-mollie-for-woocommerce' ), strtotime( $post->post_date ) );

		$messages[ self::POST_TYPE ] = [
			0  => '', // Unused. Messages start at index 1.
			1  => __( 'Payment updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			4  => __( 'Payment updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			5  => isset( $_GET['revision'] ) ? sprintf( __( 'Payment restored to revision from %s.', 'pronamic-pay-with-mollie-for-woocommerce' ), strval( wp_post_revision_title( (int) $_GET['revision'], false ) ) ) : false,
			...
			6  => __( 'Payment published.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			7  => __( 'Payment saved.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			8  => __( 'Payment submitted.', 'pronamic-pay-with-mollie-for-woocommerce' ),
			...
			9  => sprintf( __( 'Payment scheduled for: %s.', 'pronamic-pay-with-mollie-for-woocommerce' ), '<strong>' . $scheduled_date . '</strong>' ),
			...
			10 => __( 'Payment draft updated.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
	...
	private function maybe_update_payment() {
		...
		if ( ! \wp_verify_nonce( $nonce, 'pronamic_payment_update' ) ) {
			\wp_die( \esc_html__( 'Action failed. Please refresh the page and retry.', 'pronamic-pay-with-mollie-for-woocommerce' ) );
		}
