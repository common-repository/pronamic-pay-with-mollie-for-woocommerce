...
class AdminHealth {
	...
	public function debug_information( $debug_information ) {
		...
		// Time.
		$fields['time'] = [
			'label' => __( 'Time (UTC)', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'value' => esc_html( gmdate( __( 'Y/m/d g:i:s A', 'pronamic-pay-with-mollie-for-woocommerce' ) ) ),
		];

		// OpenSSL version.
		$openssl_version = __( 'Not available', 'pronamic-pay-with-mollie-for-woocommerce' );
	...
	private function get_active_plugin_integrations_debug() {
		...
		if ( empty( $active ) ) {
			$active[] = __( 'None', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
	...
	public function status_tests( $status_tests ) {
		...
		$status_tests['direct']['pronamic_pay_wordpress_version'] = [
			'label' => __( 'Pronamic Pay WordPress version test', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'test'  => [ $this, 'test_wordpress_version' ],
		];

		// Test memory limit.
		$status_tests['direct']['pronamic_pay_memory_limit'] = [
			'label' => __( 'Pronamic Pay memory limit test', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'test'  => [ $this, 'test_memory_limit' ],
		];

		// Test character set.
		$status_tests['direct']['pronamic_pay_character_set'] = [
			'label' => __( 'Pronamic Pay UTF-8 character set test', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'test'  => [ $this, 'test_character_set' ],
		];

		// Test hashing algorithms.
		$status_tests['direct']['pronamic_pay_hashing_algorithms'] = [
			'label' => __( 'Pronamic Pay hashing algorithms test', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'test'  => [ $this, 'test_hashing_algorithms' ],
		];

		// Test supported extensions.
		$status_tests['direct']['pronamic_pay_extensions_support'] = [
			'label' => __( 'Pronamic Pay extensions support test', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function test_wordpress_version() {
		...
		$result = [
			...
			'label'       => sprintf(
				...
				__( 'WordPress version is supported by Pronamic Pay (%s)', 'pronamic-pay-with-mollie-for-woocommerce' ),
				get_bloginfo( 'version' )
			),
			'description' => sprintf( '<p>%s</p>', __( 'Pronamic Pay requires at least WordPress 4.7.', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
			'badge'       => [
		...
		if ( version_compare( get_bloginfo( 'version' ), '4.7', '<' ) ) {
			...
			$result['label']  = __( 'Pronamic Pay requires at least WordPress 4.7', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
	...
	public function test_memory_limit() {
		...
		$result = [
			...
			'label'       => sprintf(
				...
				__( 'WordPress memory limit is sufficient (%s)', 'pronamic-pay-with-mollie-for-woocommerce' ),
				size_format( $memory )
			),
			'description' => sprintf( '<p>%s</p>', __( 'Pronamic Pay recommends setting the WordPress memory limit to at least 64 MB.', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'color' => 'blue',
			],
		...
		if ( $memory < 67108864 ) {
			...
			$result['label'] = sprintf(
				/* translators: %s: WordPress memory limit */
				__( 'Increase WordPress memory limit (%s) to at least 64 MB', 'pronamic-pay-with-mollie-for-woocommerce' ),
				size_format( $memory )
			);
		}
	...
	public function test_character_set() {
		...
		$result = [
			...
			'label'       => __( 'Character encoding is set to UTF-8', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'description' => sprintf( '<p>%s</p>', __( 'Pronamic Pay recommends to use the UTF-8 character encoding for payments.', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'color' => 'blue',
			],
		...
		if ( 0 !== strcasecmp( get_bloginfo( 'charset' ), 'UTF-8' ) ) {
			...
			$result['label'] = __( 'Character encoding is not set to UTF-8', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
	...
	public function test_hashing_algorithms() {
		...
		$result = [
			...
			'label'       => __( 'SHA1 hashing algorithm is available', 'pronamic-pay-with-mollie-for-woocommerce' ),
			'description' => sprintf( '<p>%s</p>', __( 'Payment gateways often use the SHA1 hashing algorithm, therefore Pronamic Pay advises to enable this hashing algorithm.', 'pronamic-pay-with-mollie-for-woocommerce' ) ),
			'badge'       => [
				'label' => __( 'Payments', 'pronamic-pay-with-mollie-for-woocommerce' ),
				'color' => 'blue',
			],
		...
		if ( ! in_array( 'sha1', $algorithms, true ) ) {
			...
			$result['label'] = __( 'SHA1 hashing algorithm is not available for Pronamic Pay', 'pronamic-pay-with-mollie-for-woocommerce' );
		}
	...
	public function test_extensions_support() {
		...
		if ( 0 !== count( $untested_plugin_versions ) ) {
			...
			$extensions_list_text .= sprintf(
				'<p>%1$s</p><p>%2$s</p>',
				__( 'Untested plugin versions:', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		if ( 0 !== count( $outdated_plugin_versions ) ) {
			...
			$extensions_list_text .= sprintf(
				'<p>%1$s</p><p>%2$s</p>',
				__( 'Outdated unsupported plugin versions for which payments can not be processed:', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		if ( 0 !== count( $supported_extensions ) ) {
			$extensions_list_text .= sprintf(
				'<p>%1$s</p><p>%2$s</p>',
				__( 'Supported plugin versions:', 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		$label = sprintf(
			...
			__( '%s extensions are compatible', 'pronamic-pay-with-mollie-for-woocommerce' ),
			__( 'Pronamic Pay', 'pronamic-pay-with-mollie-for-woocommerce' )
		);

		$description_text = __( 'Pronamic Pay uses extensions to integrate with form, booking and other e-commerce plugins. All extensions support the currently activated plugin versions.', 'pronamic-pay-with-mollie-for-woocommerce' );

		if ( 'good' !== $status ) {
			$label = sprintf(
				/* translators: %s: plugin name */
				__( '%s extensions are incompatible', 'pronamic-pay-with-mollie-for-woocommerce' ),
				__( 'Pronamic Pay', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
