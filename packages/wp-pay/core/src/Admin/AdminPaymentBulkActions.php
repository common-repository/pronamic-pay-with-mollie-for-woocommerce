...
class AdminPaymentBulkActions {
	...
	public function bulk_actions( $bulk_actions ) {
		...
		$bulk_actions['pronamic_payment_check_status'] = __( 'Check Payment Status', 'pronamic-pay-with-mollie-for-woocommerce' );
	...
	public function admin_notices() {
		...
		if ( $updated > 0 ) {
			...
			$message = sprintf( _n( '%s payment updated.', '%s payments updated.', $updated, 'pronamic-pay-with-mollie-for-woocommerce' ), number_format_i18n( $updated ) );
		...
		if ( $skipped > 0 ) {
			$message = sprintf(
				...
				_n( '%s payment is not updated because it already has a final payment status.', '%s payments are not updated because they already have a final payment status.', $skipped, 'pronamic-pay-with-mollie-for-woocommerce' ),
		...
		if ( ! empty( $gateways ) ) {
			...
			$message = sprintf(
				...
				__( 'Requesting the current payment status is unsupported by %s.', 'pronamic-pay-with-mollie-for-woocommerce' ),
				implode( ', ', $titles )
			);
