...
class Util {
	...
	public static function format_date_interval( DateInterval $date_interval ) {
		...
		foreach ( [ 'y', 'm', 'd', 'h', 'i', 's' ] as $period ) {
			...
			switch ( $period ) {
				case 'y':
					...
					$format = _n( '%s year', '%s years', $value, 'pronamic-pay-with-mollie-for-woocommerce' );

					break;
				case 'm':
					/* translators: %s: number of months */
					$format = _n( '%s month', '%s months', $value, 'pronamic-pay-with-mollie-for-woocommerce' );

					break;
				case 'd':
					/* translators: %s: number of days */
					$format = _n( '%s day', '%s days', $value, 'pronamic-pay-with-mollie-for-woocommerce' );

					break;
				case 'h':
					/* translators: %s: number of hours */
					$format = _n( '%s hour', '%s hours', $value, 'pronamic-pay-with-mollie-for-woocommerce' );

					break;
				case 'i':
					/* translators: %s: number of minutes */
					$format = _n( '%s minute', '%s minutes', $value, 'pronamic-pay-with-mollie-for-woocommerce' );

					break;
				case 's':
					/* translators: %s: number of seconds */
					$format = _n( '%s second', '%s seconds', $value, 'pronamic-pay-with-mollie-for-woocommerce' );
		...
		if ( count( $periods ) > 1 ) {
			...
			return sprintf(
				...
				__( '%1$s and %2$s', 'pronamic-pay-with-mollie-for-woocommerce' ),
				$formatted,
				$last_period
			);
		}
	...
	public static function format_recurrences( DateInterval $date_interval ) {
		...
		return sprintf(
			...
			__( 'Every %s', 'pronamic-pay-with-mollie-for-woocommerce' ),
			$formatted_interval
		);
	}
	...
	public static function format_interval( $interval, $period ) {
		switch ( $period ) {
			...
			case 'days':
				...
				return sprintf( _n( 'Every %s day', 'Every %s days', $interval, 'pronamic-pay-with-mollie-for-woocommerce' ), $interval );
			case 'W':
			case 'week':
			case 'weeks':
				/* translators: %s: interval */
				return sprintf( _n( 'Every %s week', 'Every %s weeks', $interval, 'pronamic-pay-with-mollie-for-woocommerce' ), $interval );
			case 'M':
			case 'month':
			case 'months':
				/* translators: %s: interval */
				return sprintf( _n( 'Every %s month', 'Every %s months', $interval, 'pronamic-pay-with-mollie-for-woocommerce' ), $interval );
			case 'Y':
			case 'year':
			case 'years':
				/* translators: %s: interval */
				return sprintf( _n( 'Every %s year', 'Every %s years', $interval, 'pronamic-pay-with-mollie-for-woocommerce' ), $interval );
		}
	...
	public static function format_frequency( $frequency ) {
		if ( empty( $frequency ) ) {
			return _x( 'Unlimited', 'Recurring payment', 'pronamic-pay-with-mollie-for-woocommerce' );
		}

		/* translators: %s: frequency */
		return sprintf( _n( '%s period', '%s periods', $frequency, 'pronamic-pay-with-mollie-for-woocommerce' ), $frequency );
	}
