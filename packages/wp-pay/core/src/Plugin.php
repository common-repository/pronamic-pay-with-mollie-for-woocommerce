...
class Plugin {
	...
	public function __construct( $args = [] ) {
		...
		$payment_method_afterpay_nl->descriptions = [           
			 ...
			 */
			'default' => \__( 'AfterPay is one of the largest and most popular post-payment system in the Benelux. Millions of Dutch and Belgians use AfterPay to pay for products.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
		...
		$payment_method_afterpay_com->descriptions = [
			 ...
			 */
			'default' => \__( 'Afterpay is a popular buy now, pay later service in Australia, New Zealand, the United States, and Canada.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
		...
		$description_template = \__( 'By using this payment method you authorize us via %s to debit payments from your bank account.', 'pronamic-pay-with-mollie-for-woocommerce' );
		...
		$payment_method_ideal->descriptions = [
			'customer' => \__( 'With iDEAL you can easily pay online in the secure environment of your own bank.', 'pronamic-pay-with-mollie-for-woocommerce' ),
		];
		...
		$payment_method_riverty->descriptions = [
			'default'  => \__( 'Riverty (formerly AfterPay) is a payment service that allows customers to pay after receiving the product.', 'pronamic-pay-with-mollie-for-woocommerce' ),
	...
	public function datetime_format( $format ) {
		$format = _x( 'D j M Y \a\t H:i', 'default datetime format', 'pronamic-pay-with-mollie-for-woocommerce' );
	...
	public static function get_default_error_message() {
		return __( 'Something went wrong with the payment. Please try again or pay another way.', 'pronamic-pay-with-mollie-for-woocommerce' );
	}
	...
	public static function get_config_select_options( $payment_method = null ) {
		...
		$options = [ __( '— Select Configuration —', 'pronamic-pay-with-mollie-for-woocommerce' ) ];
		...
		if ( null === $gateway ) {
			$payment->add_note(
				\sprintf(
					...
					\__( 'Payment failed because gateway configuration with ID `%d` does not exist.', 'pronamic-pay-with-mollie-for-woocommerce' ),
					$config_id
				)
			);
	...
	public static function create_refund( Refund $refund ) {
		...
		if ( null === $gateway ) {
			throw new \Exception(
				\esc_html__( 'Unable to process refund as gateway could not be found.', 'pronamic-pay-with-mollie-for-woocommerce' )
			);
		}
